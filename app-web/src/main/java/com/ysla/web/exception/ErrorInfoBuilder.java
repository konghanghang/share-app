...
public class ErrorInfoBuilder implements HandlerExceptionResolver, Ordered {
     ...
     * 错误KEY
     */
    private final static String ERROR_NAME = "ysla.error";
     ...
     * 构建错误信息.
     */
    public JsonApi getErrorInfo(HttpServletRequest request) {
     ...
     * 构建错误信息.
     */
    public JsonApi getErrorInfo(HttpServletRequest request, Throwable error) {
        System.err.println(getStackTraceInfo(error, isIncludeStackTrace(request)));
        String message = new StringBuilder("url: ")
                .append(request.getAttribute(WebUtils.ERROR_REQUEST_URI_ATTRIBUTE))
                .append(" -- ")
                .append(error.toString())
                .toString();
        return JsonApi.isFail(getHttpStatus(request).value())
                .message(message)
                .data(getHttpStatus(request).getReasonPhrase());
    }
